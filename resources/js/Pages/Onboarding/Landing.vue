...
<template>
    ...
    <section class="first flex flex-col items-center pb-32">
        ...
        <transition name="smooth" mode="out-in">
            <div v-if="!setup">
                <div
                    class="absolute right-20 top-12 bg-gray-200 px-3 py-1 rounded-full opacity-70 flex items-center gap-1 text-sm"
                >
                    <p>
                        You are logged in as <b>{{ attrs.user.name }}</b>
                    </p>
                    <p
                        class="italic cursor-pointer hover:underline"
                        @click="logout"
                    >
                        Log out
                    </p>
                ...
                <div class="flex items-center max-w-4xl mt-16">
                    <div class="w-1/2">
                        ...
                        <p>
                            Key Manager gives your team a home - a place where
                            they manage virtual keys together. To create a new
                            team, click the button below.
                        </p>
                        <button
                            @click="createTeam"
    ...
    <section v-if="!setup" class="second flex flex-col items-center pb-32">
        <div class="w-128">
            ...
            <div class="w-full h-96 border border-gray-200 shadow-lg rounded">
                ...
                <div
                    v-if="invitations == 0"
                    class="text-center mt-20 font-bold"
                >
                ...
                <div>
                    <div
                        ...
                        class="z-20 p-7 text-sm border-b border-gray-200 font-bold flex items-center justify-between hover:bg-gray-50 cursor-pointer"
...
export default {
    ...
    methods: {
        async createTeam() {
            await axios
                .post(`/api/teams/create`, {
                    name:
                        this.attrs.user.name.split(" ", 2)[0] + "'s" + " team",
                })
                .then((response) => {
                    this.renderSetup();
        ...
        getInvitations() {
            axios
                .get(`/teams/invitations/${this.attrs.user.id}`)
                .then((response) => {
