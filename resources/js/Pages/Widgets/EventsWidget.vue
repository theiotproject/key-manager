...
<!--                  </th>-->
        ...
        >
          <p class="text-xl text-gray-600">You don't have any Events</p>
        </div>
        <div v-if="events.length == 10" class="mt-5 w-full flex justify-center">
          <Link
...
export default {
  ...
  methods: {
    getEvents() {
      axios
        .get(
          `/events/teamId/${this.localAttrs.user.current_team_id}/limit/10/pagination/0`
        )
        .then((response) => {
